<x-app-layout>
        ...
        <div class="container saas-wide">
            ...
            <div class="flex items-center gap-2 mb-4 border-b border-slate-200">
                <a href="{{ route('user.active') }}"
                    ...
                    <i class="fas fa-user-check text-xs text-emerald-600"></i>
                ...
                <div class="p-4 sm:p-5 border-b border-slate-100 flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-slate-50/30">
                    <div>
                        <h2 class="text-xs sm:text-sm font-bold uppercase tracking-wider text-slate-800 m-0">
                            {{ __('Lista de Utilizadores Ativos') }}
                        </h2>
                    </div>
                                    ...
                                    <tr>
                                        <td class="py-3">
                                            <div class="flex items-center gap-3">
                                                <div class="w-9 h-9 rounded-full bg-emerald-100 text-emerald-800 font-bold text-xs flex items-center justify-center shrink-0 border border-emerald-200/60 shadow-2xs">
                                        ...
                                        <td class="py-3 text-right">
                                            <div class="inline-flex items-center justify-end gap-1.5 flex-wrap">
                                                @if ($user->type !== 'administrative' && !empty($user->ciencia_vitae) && $user->authorInformation)
                                                    <button type="button"
                                                        class="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-semibold text-emerald-800 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200/80 rounded-lg transition-colors cursor-pointer update-author-cv"
                                                        data-author-id="{{ $user->authorInformation->id }}"
                                                        title="{{ __('Atualizar dados via Ciência Vitae') }}">
                                                ...
                                                <form method="POST" action="{{ route('user.deactivate', $user->id) }}" class="inline-block m-0">
                                                    @csrf
                                                    <button type="submit"
                                                        class="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-semibold text-amber-800 bg-amber-50 hover:bg-amber-100 border border-amber-200 rounded-lg transition-colors cursor-pointer"
                                                        title="{{ __('Desativar conta') }}">
                                                        <i class="fas fa-user-slash text-[10px]"></i>
                                                        <span>{{ __('Desativar') }}</span>
                                                    </button>
                                                </form>
                                                ...
                                                @if ($user->id !== auth()->id())
                                                    <form method="POST" action="{{ route('user.destroy', $user->id) }}" class="inline-block m-0"
                                                        ...
                                                        <button type="submit"
                                                            ...
                                                            title="{{ __('Remover conta permanentemente') }}">
        ...
        function initializeDataTable() {
            ...
            const TABLE = $('#users_table').DataTable({
                ...
                language: {
                    ...
                    emptyTable: "{{ __('Nenhum utilizador registado') }}",
    ...
    @pushOnce('scripts')
        <script src="{{ asset('javascript/authors/updateAuthorFromAdmin.js') }}" defer></script>
    @endPushOnce
